...
export default function HomeStaff() {
  ...
  const TILES = [
    { icon: "🪑", num: `${stats?.tavoliLiberi}/${stats?.tavoliTotali}`, label: "Tavoli liberi",    path: "/staff/tavoli" },
    { icon: "📅", num: stats?.prenotazioniOggi,                         label: "Prenotazioni oggi", path: "/prenotazioni" },
    { icon: "🍳", num: stats?.ordiniAttivi,                             label: "Ordini attivi",     path: "/staff/cucina" },
    { icon: "🍽", num: stats?.piatti,                                   label: "Piatti nel menu",   path: "/staff/menu" },
  ];
    ...
    <div>
      ...
      <div className="card" style={{ marginTop: "2rem" }}>
        ...
        <div style={{ display: "flex", gap: ".75rem", flexWrap: "wrap" }}>
          <button className="btn btn-primary" onClick={() => navigate("/staff/cucina")}>
            👨‍🍳 Dashboard cucina
          </button>
          <button className="btn btn-ghost" onClick={() => navigate("/staff/tavoli")}>
            🪑 Gestione tavoli
          </button>
          <button className="btn btn-ghost" onClick={() => navigate("/staff/menu")}>
            🍽 Gestione menu
          </button>
        </div>
      </div>
